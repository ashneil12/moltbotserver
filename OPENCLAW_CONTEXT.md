...
67. **ESM mock pattern** — All custom test files must use `vi.spyOn(module, "export").mockImplementation(...)` instead of `vi.hoisted` + `vi.mock()`. In this pnpm `node-linker: hoisted` workspace, `vi.mock()` silently fails to intercept already-cached ESM bindings. If tests suddenly call real implementations after an upstream sync, check for `vi.mock()` usage and convert to `vi.spyOn()`.
68. **QMD collection collision awareness** — `resolveDefaultWorkspaceCollection()` in `backend-config.ts` always adds a `workspace-{agentId}` collection. Test configs with `paths: [{ name: "workspace" }]` will produce a collision resulting in `workspace-main-2`. Update search/query call expectations and collection add/remove counts accordingly. The retry flag is per-run (not per-collection): first search failure sets the flag; subsequent collections go directly to `query`. See `src/memory/qmd-manager.test.ts` file-level JSDoc for details.
69. **Pre-existing tsgo type errors (upstream merge leftovers)** — After the upstream sync, `pnpm tsgo` reports 4 unresolved type errors NOT introduced by our cleanup. Use `--no-verify` temporarily; fix by updating call sites to match upstream type signatures:
    - `src/secrets/runtime.ts:102` — Expected 2 arguments, got 3
    - `src/security/audit-extra.sync.ts:333` — `tavily` removed from web search provider union
    - `src/security/audit.test.ts:865` — `strictInlineEval` removed from `ExecToolConfig`
    - `src/tts/tts.ts:831` — `isCompactionNotice` removed from `ReplyPayload`
