...
| `enforce-config.mjs`                               | Model normalization, reflection intervals, cron job seeding, tool loop detection enablement                                                                                                                                          |
...
| `scripts/sandbox-browser-entrypoint.sh`            | Custom browser container entrypoint — includes `OPENCLAW_BROWSER_NOVNC_NO_AUTH` env var (disables VNC password when `1`; Caddy gateway token provides auth)                                                                          |
...
| `src/agents/workspace.ts`                 | `resolveHumanModeEnabled()`, `resolveHonchoEnabled()`, Honcho conditionals, `stripHonchoConditionals()`, `removeHumanModeSectionFromSoul()`; `preLoad` callback on `WorkspaceBootstrapFile` for `rebuildKnowledgeIndex` hook; file content cache with mtime invalidation |
...
6. **Build verification** — `npm install && npm run build`
