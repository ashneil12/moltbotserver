...
| `enforce-config.mjs`              | Model normalization, 4-tier reflection patching, cron job seeding (`MAIN_ONLY_JOBS`), tool loop detection, `tools.profile = "full"` enforcement, per-agent browser profiles/containers, `browser` in `alsoAllow`, Bailian auto-registration, LCM version-aware enforcement, skill-evolution cron, cron payload sync, cron schedule redesign (interval→fixed cron), `MAIN_ONLY_JOBS` expansion |
...
| `scripts/add-cron.mjs`                                                                        | **NEW** — Deterministic cron management (validates, deduplicates, auto-resolves delivery)                           |
...
| `src/gateway/server-cron.ts`                      | `startPreResetFlushTimer`/`startDiaryArchiveTimer` integration                                                                                                                                              |
...
| `src/auto-reply/reply/session.ts`                 | `preResetFlushAt` clear, `persistSessionContextOnReset()`, `indexTranscriptForSearch()`                                                                                                                     |
