...
33. **QMD searchMode** — `grep -c 'OPENCLAW_QMD_SEARCH_MODE' enforce-config.mjs` ≥ 1. Defaults to `search` (BM25). Set to `vsearch` for hybrid vector+BM25 if Gemini embeddings are available.
