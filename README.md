...
Every agent is automatically provisioned with **17 cron jobs** covering reflection, maintenance, memory enrichment, and autonomous building. These are seeded at agent creation via `enforce-config.mjs cron-seed` and can be customized per agent.

Some jobs are **main-agent only** (`MAIN_ONLY_JOBS`) — sub-agents get the core set without duplicating platform-level tasks like backups or global fact extraction.
...
| **Session & Reply** |  ~200 | Session health, session search, context summary, freshness, noise patterns, trajectory compression, tool stats, typing   |
| **Health Sentinel** |   ~66 | Core orchestrator, Phase 2 (history/trends/playbooks), Phase 3 (incidents/inbox/TTL), sidecar probes                     |
| **Infrastructure**  |   ~30 | Ephemeral path detection, atomic file utilities, heartbeat runner                                                        |
| **Memory**          |   ~40 | Source-aware ranking, knowledge base indexer                                                                             |
