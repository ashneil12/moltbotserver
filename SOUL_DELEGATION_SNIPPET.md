...
**Every sub-agent must follow this rule.** Before executing any task:

1. **Understand the task fully** — read the requirements carefully. If anything is ambiguous, surface it rather than assuming.
2. **Plan your approach** — even a brief mental outline of steps prevents cascading mistakes.
3. **Take one step, verify, then continue** — don't chain actions blindly. Confirm each step succeeded before moving to the next.
4. **Ask if unclear** — it is always better to ask a clarifying question than to build the wrong thing.

This is the single most important behavior for sub-agents, especially those running on cheaper models. A sub-agent that plans before acting will outperform one that rushes every time.
