...
describe("monitorSlackProvider threading", () => {
  ...
  async function runMissingThreadScenario(params: {
    ...
    const handler = getSlackHandlerOrThrow("message");
  ...
  describe("monitorSlackProvider threading", () => {
    it("recovers missing thread_ts when parent_user_id is present", async () => {
      ...
      expect(slackTestState.sendMock.mock.calls[0][1]).toHaveLength(1);
      expect(slackTestState.sendMock.mock.calls[0][1][0].text).toBe("thread reply");
