...
Use a **Trigger Script** (`triggerScript`). This is a lightweight script (Bash, Python, JS) that runs _before_ the Agent.
...
- _Examples:_ "Check if price < 150", "Check if file exists", "Grep for keyword".
...
1.  **Write the Script**: Create a script in `scripts/triggers/` (e.g., `scripts/triggers/check-stock.sh`).
    - _Must_ verify prerequisites (curl, jq, etc.).
    - _Must_ print meaningful output ONLY when action is needed.
    - _Must_ be silent (or exit 1) when nothing is needed.
