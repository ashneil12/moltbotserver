...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    cron_heal:
      "Diagnose and self-heal cron/disk issues. Use when Health Sentinel alerts you or during proactive health checks. Actions: diagnose, re-enable, adjust-schedule, force-run, cleanup-disk, rollback, journal. All actions are journaled with auto-rollback.",
  ...
  const lines = [
    ...
    availableTools.has("web_search") && availableTools.has("browser") && !isMinimal
      ? [
          ...
          "- When `web_search` results are insufficient or you need deeper detail from a specific page, escalate to `web_fetch` (which has built-in stealth scraping fallback for bot-protected sites) or the browser sidecar.",
