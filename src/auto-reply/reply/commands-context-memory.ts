...
import { forceRotation, clearRotationState } from "../../context/rotation-service.js";
...
export const handleFreshCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  // Force rotation to move current messages to history
  try {
    await forceRotation(params.workspaceDir, params.sessionKey);
  } catch {
    // Rotation disabled or failed - that's okay
  }

  // Clear local rotation state to reset counters
  clearRotationState(params.sessionKey);
