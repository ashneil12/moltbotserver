...
export const SEARCH_PROVIDER_OPTIONS: readonly SearchProviderEntry[] = [
  ...
  },
  {
    ...
    hint: "Free · no API key · metasearch aggregator (auto-detected from Docker)",
    envKeys: [],
    placeholder: "",
...
function rawKeyValue(config: OpenClawConfig, provider: SearchProvider): unknown {
  ...
  switch (provider) {
    ...
    case "searxng":
      return undefined; // SearXNG doesn't use API keys
  }
}
...
export function applySearchKey(
  ...
  switch (provider) {
    ...
    case "perplexity":
      ...
      break;
    case "searxng":
      // SearXNG doesn't use API keys — provider-only, no key to store
      break;
  }
  return {
    ...config,
