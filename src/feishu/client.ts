...
export function getFeishuClient(accountIdOrAppId?: string, explicitAppSecret?: string) {
  ...
  if (!appSecret && feishuCfg?.accounts) {
    if (isAppId) {
      ...
      // oxlint-disable-next-line no-unnecessary-type-assertion -- tsgo needs this cast
      for (const [, acc] of Object.entries(feishuCfg.accounts) as [
        string,
        { appId?: string; appSecret?: string; appSecretFile?: string; domain?: string },
      ][]) {
